...
app.use(
  cors({
    ...
    allowMethods: ["GET", "POST", "DELETE", "PUT", "OPTIONS"], // 设置所允许的 HTTP 请求方法
...
router.post(
  ...
  koaBody({
    ...
    formidable: {
      ...
      onFileBegin: (name, file) => {
        ...
        currChunk = { filename, fileHash, index };
  ...
  async (ctx) => {
    ...
    ctx.body = JSON.stringify({ code: 2000, message: "upload successfully" });
  }
);
...
router.post("/mergeChunks", async (ctx) => {
  ...
  ctx.body = JSON.stringify({
    ...
    message: "merge chunks successfully",
  });
});
...
const mergeFileChunk = async (filePath, filename, size) => {
  ...
  console.log("chunkPaths :>> ", chunkPaths); // hk-log
